-- Auditoria do card de Custo (Analytics da Ana) — itens 1+2+3.
-- Aplicado em prod via MCP em 2026-06-30; versionado aqui pra reprodutibilidade.
--   1) Incluir Mercado Livre (marketplace_questions) no custo — antes o rollup só somava public.messages.
--   2) Separar tokens input/output e custear com taxa real por modelo:
--        WA/IG = claude-sonnet-4-6 (US$3/1M in, US$15/1M out)
--        ML    = claude-opus-4-6  (US$5/1M in, US$25/1M out)
--      Linhas legadas (só tokens_used combinado) usam blended ~85/15 (Sonnet 4.80, Opus 8.00).
--   3) Câmbio USD->BRL configurável (agent_config.usd_brl_rate, default 5.50); estimated_cost_brl no rollup.
-- Idempotente: ADD COLUMN IF NOT EXISTS + INSERT WHERE NOT EXISTS + CREATE OR REPLACE.
...
ALTER TABLE public.marketplace_questions ADD COLUMN IF NOT EXISTS tokens_in  integer;
ALTER TABLE public.marketplace_questions ADD COLUMN IF NOT EXISTS tokens_out integer;

ALTER TABLE public.analytics_daily ADD COLUMN IF NOT EXISTS estimated_cost_brl numeric;
ALTER TABLE public.analytics_daily ADD COLUMN IF NOT EXISTS total_tokens_in  integer;
ALTER TABLE public.analytics_daily ADD COLUMN IF NOT EXISTS total_tokens_out integer;

INSERT INTO public.agent_config (config_key, config_value)
SELECT 'usd_brl_rate', '5.50'
WHERE NOT EXISTS (SELECT 1 FROM public.agent_config WHERE config_key = 'usd_brl_rate');
  ...
  msg AS (
    SELECT created_at::date AS d,
           ...
           sum(CASE WHEN tokens_in IS NOT NULL OR tokens_out IS NOT NULL
                    THEN COALESCE(tokens_in,0)*3.0 + COALESCE(tokens_out,0)*15.0
